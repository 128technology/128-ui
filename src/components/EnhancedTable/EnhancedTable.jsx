...
class EnhancedTable extends React.Component {
  ...
  handleRequestSort = (dataKey, isNumeric) => {
    ...
    this.setState(({ orderBy, orderDirection, cols, data }) => {
  ...
  validateColumnKeys(columns) {
    const missingKeys = _.map(_.filter(_.uniqBy(columns, 'key'), x => !_.has(x, 'key')), x => x.label);
  ...
  render() {
    const { orderBy, orderDirection, isNumeric, cols, data } = this.state;
    ...
    return (
      <Paper style={{ height: this.getTableHeight(), width: '100%', boxShadow: 'none' }}>
        {loading && <Loading />}
        {data && (
          <AutoSizer onResize={_.debounce(this.recomputeGridSize, 200)}>
            {({ height, width }) => (
              <MultiGrid
                ref={this.setRef}
                data={data}
